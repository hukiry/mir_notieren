...
namespace Hukiry.Editor
{
    ...
    /// 位图字创建GUI界面
    /// </summary>
    public class BitmapFontCreateEditorWindow : EditorWindow,IHasCustomMenu
    {
        /// <summary>
        /// 打开
        /// </summary>
        ...
        void IHasCustomMenu.AddItemsToMenu(GenericMenu menu)
        {
            menu.AddItem(new GUIContent("定位脚本"), false, () => {
                Hukiry.HukiryUtilEditor.LocationObject<MonoScript>(nameof(BitmapFontCreateEditorWindow));
            });
        }

   
        [MenuItem("CONTEXT/Font/编辑字体", false, 100)]
        static void SetFontEditor(MenuCommand command)
        {
            Font importer = command.context as Font;
            Hukiry.HukiryUtilEditor.LocationObject<MonoScript>(nameof(BitmapFontCreateEditorWindow));
        }
    }
}