...
namespace Hukiry
{
    public class HukiryGLDraw
	{
		...
		public static void CreateMaterial()
		{
			...
			LineShader = Shader.Find("Unlit/Colored Transparent");
		...
		private static void DrawCurve( Vector3[] allPoints, Vector2[] allNormals, Color[] allColors, int pointCount )
		{
			CreateMaterial();
		...
		public static Rect DrawLine(Vector2 start, Vector2 end, Color color)
		{
			int segments = Mathf.FloorToInt((start - end).magnitude / 20) * 3; // Three segments per distance of 20
			return DrawBezier(start, start, end, end, color, 1, segments, 1);
		}
